...
\begin{abstract}
Large Language Models (LLMs) have transformed natural language processing, yet their tendency to generate factually incorrect information---known as hallucination---poses significant risks for applications requiring factual accuracy. This thesis presents a comparative analysis of three state-of-the-art LLMs (GPT-4, Gemini, and DeepSeek) within the TruthLens framework, a decoupled two-phase architecture for automated fact-checking that separates claim extraction from search-augmented verification.

Using the BingCheck dataset, we evaluated model performance across 1,950 API calls: $N=150$ sentences for extraction ($k=3$ runs) and $N=100$ claims for verification ($k=2$ runs). The results reveal distinct behavioral patterns. For claim extraction, GPT-4 achieves the highest F1-Score (80.4\%) through a balanced precision-recall strategy, while Gemini and DeepSeek adopt conservative approaches with high precision (89.5\% and 95.8\%) but significantly reduced recall (40.2\% and 49.6\%). For verification, Gemini achieves the highest accuracy (82.0\%), DeepSeek the highest Macro-F1 (55.9\%), and GPT-4 the most deterministic outputs. All models struggle with minority class identification, with a 25--30 percentage point gap between accuracy and Macro-F1 scores.
...
\section{Structure of the Thesis}
    ...
    \textbf{Chapter \ref{ch:results}} presents the quantitative results of the experiments. It offers a statistical comparison of GPT-4, Gemini, and DeepSeek across both extraction and verification phases, analyzing accuracy, reliability, and inter-model agreement across multiple experimental runs.
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|X|X|}
        ...
        \hline
        GPT-4 & 80.2\% $\pm$ 1.7\% & 83.1\% $\pm$ 2.5\% & 77.8\% $\pm$ 0.7\% & 80.4\% $\pm$ 1.5\% \\
        \hline
        Gemini & 66.4\% $\pm$ 1.5\% & 89.5\% $\pm$ 1.8\% & 40.2\% $\pm$ 2.7\% & 55.4\% $\pm$ 2.8\% \\
        \hline
        DeepSeek & 72.7\% $\pm$ 2.9\% & 95.8\% $\pm$ 1.7\% & 49.6\% $\pm$ 4.9\% & 65.3\% $\pm$ 4.6\% \\
        \hline
    \end{tabularx}
...
\textbf{Key Finding:} GPT-4 achieves the highest F1-Score (80.4\%), demonstrating a balanced trade-off between precision and recall. In contrast, Gemini and DeepSeek exhibit a \textit{conservative extraction strategy}---achieving exceptionally high precision (89.5\% and 95.8\% respectively) at the cost of significantly reduced recall (40.2\% and 49.6\%).

This pattern has important practical implications: while Gemini and DeepSeek rarely misclassify non-factual content as claims (high precision), they fail to identify approximately half of the actual factual claims present in the text (low recall). For a fact-checking pipeline where \textit{coverage} is critical, this represents a significant limitation.
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|X|}
        ...
        \hline
        GPT-4 & 82.9\% $\pm$ 2.9\% & 77.5\% $\pm$ 1.1\% & 80.1\% $\pm$ 1.9\% \\
        \hline
        Gemini & 94.9\% $\pm$ 0.8\% & 59.4\% $\pm$ 1.1\% & 73.1\% $\pm$ 1.0\% \\
        \hline
        DeepSeek & 97.7\% $\pm$ 0.8\% & 64.2\% $\pm$ 2.4\% & 77.5\% $\pm$ 2.0\% \\
        \hline
    \end{tabularx}
...
\begin{figure}[H]
    ...
    \caption{Precision-Recall trade-off for claim extraction. GPT-4 occupies the balanced region near F1=0.8, while Gemini and DeepSeek cluster in the high-precision, low-recall quadrant.}
...
\begin{figure}[H]
    ...
    \caption{Confusion matrices for claim extraction (averaged across $k=3$ runs). GPT-4 shows balanced errors, while Gemini and DeepSeek exhibit high false negative counts (39--47 missed claims).}
    \label{fig:confusion_matrices}
\end{figure}

\textbf{Interpretation:}
\begin{itemize}
    \item \textbf{GPT-4:} Exhibits relatively balanced error distribution with 12 false positives and 17 false negatives on average.
    \item \textbf{Gemini:} Shows extreme conservatism with only 4 false positives but 47 false negatives---missing over half of actual claims.
    \item \textbf{DeepSeek:} Similar conservative pattern with 2 false positives and 39 false negatives.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Run-to-run variability for extraction phase ($k=3$ runs). GPT-4 demonstrates the most consistent performance with minimal variance.}
    \label{fig:extraction_reliability}
\end{figure}

GPT-4 demonstrates the most stable performance with the smallest standard deviation ($\pm$1.5\% for F1-Score), while DeepSeek exhibits the highest variability ($\pm$4.6\%). This variability in DeepSeek's performance may be attributed to its less deterministic response patterns when processing ambiguous sentences.
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|X|X|X|}
        ...
        \hline
        GPT-4 & 79.0\% $\pm$ 0.0\% & 51.2\% $\pm$ 3.9\% & 89.2\% & 36.7\% & 27.8\% \\
        \hline
        Gemini & 82.0\% $\pm$ 1.4\% & 52.7\% $\pm$ 2.0\% & 90.9\% & 34.8\% & 32.5\% \\
        \hline
        DeepSeek & 77.5\% $\pm$ 2.1\% & 55.9\% $\pm$ 0.4\% & 87.3\% & 40.0\% & 40.5\% \\
        \hline
    \end{tabularx}
...
\textbf{Key Finding:} Gemini achieves the highest accuracy (82.0\%), while DeepSeek achieves the highest Macro-F1 score (55.9\%). The divergence between these metrics is explained by the severe class imbalance in the benchmark dataset.
...
\begin{itemize}
    \item GPT-4 and DeepSeek show the highest pairwise agreement (86\%), suggesting similar reasoning patterns.
    \item Gemini and DeepSeek show the lowest agreement (73\%), indicating divergent decision boundaries.
...
\begin{figure}[H]
    ...
    \caption{Run-to-run variability for verification phase ($k=2$ runs). GPT-4 produces identical results across runs, while DeepSeek shows the highest variability.}
    \label{fig:verification_reliability}
\end{figure}

GPT-4 demonstrates perfect consistency with 0.0\% standard deviation in accuracy, producing identical verdicts across independent runs. Gemini shows moderate variability ($\pm$1.4\%), while DeepSeek exhibits the highest variability ($\pm$2.1\%). This pattern mirrors the extraction phase results, confirming that GPT-4 is the most deterministic model across both tasks.
...
\begin{figure}[H]
    ...
    \caption{Radar chart comparing model performance across all key metrics. GPT-4 demonstrates the most balanced profile, while Gemini and DeepSeek show specialized strengths.}
    \label{fig:radar}
\end{figure}
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|X|}
        \hline
        \textbf{Criterion} & \textbf{GPT-4} & \textbf{Gemini} & \textbf{DeepSeek} \\
        \hline
...
This chapter interprets the empirical findings presented in Chapter \ref{ch:results}, contextualizes them within the broader literature, and provides a qualitative analysis of model failure modes. We adopt a useful analogy throughout: if these LLMs were members of a legal research team, \textbf{GPT-4} would be the balanced researcher who finds most evidence, while \textbf{Gemini} and \textbf{DeepSeek} are the overly cautious lawyers who only cite a fact if they are highly confident---even if it means ignoring half of the relevant material.

\section{Comparative Analysis of LLMs}

\subsection{Best Performing LLM for Extraction}

The extraction phase results clearly establish \textbf{GPT-4} as the most effective model for claim extraction, achieving an F1-Score of 80.4\% compared to 55.4\% (Gemini) and 65.3\% (DeepSeek). This superiority stems from GPT-4's balanced approach to the precision-recall trade-off.

The critical distinction lies in \textit{extraction strategy}:
\begin{itemize}
    \item \textbf{GPT-4 (Balanced):} Achieves 83.1\% precision and 77.8\% recall, correctly identifying approximately 8 out of 10 actual factual claims while maintaining high precision.
    \item \textbf{Gemini \& DeepSeek (Conservative):} Achieve exceptionally high precision (89.5\% and 95.8\% respectively) but at severe cost to recall (40.2\% and 49.6\%). These models fail to identify approximately half of all factual claims present in the text.
\end{itemize}

This conservative behavior has significant practical implications. In a fact-checking pipeline, a missed claim (false negative) represents information that will never be verified---a silent failure that undermines the system's utility. The high specificity of Gemini (94.9\%) and DeepSeek (97.7\%) indicates excellent ability to filter out subjective content, but their low Negative Predictive Value (NPV: 59.4\% and 64.2\%) reveals that when these models classify a sentence as ``non-factual,'' there is approximately a 35--40\% chance it actually contained a verifiable claim.

\textbf{Recommendation:} For extraction tasks where comprehensive coverage is important, GPT-4 is the clear choice. Gemini and DeepSeek may be appropriate only when precision is paramount and missing claims is acceptable.
...
DeepSeek's higher variability may be attributed to its sensitivity to prompt formatting. As noted in Chapter \ref{ch:implementation}, DeepSeek required custom middleware to enforce structured JSON output, suggesting that open-weights models may require additional engineering effort to achieve production-level consistency.
...
\textbf{1. Over-Filtering of Context-Dependent Claims:} Gemini and DeepSeek frequently rejected claims that required contextual interpretation. For example, sentences containing demonstrative pronouns (``This process...'', ``These results...'') were often classified as non-factual despite containing verifiable information.

\textbf{2. Ambiguous Factuality Boundaries:} All models struggled with claims at the boundary between factual and subjective content. Statements like ``Improving sleep quality is a common goal for many people'' were inconsistently classified---GPT-4 tended to accept such claims as factual (high recall), while Gemini and DeepSeek rejected them (high precision).
...
\begin{itemize}
    \item GPT-4: 10 cases (10\% of claims)
    \item Gemini: 2 cases (2\% of claims)
    \item DeepSeek: 18 cases (18\% of claims)
\end{itemize}

Interestingly, this pattern is \textit{inverse} to extraction behavior: DeepSeek, which was the most conservative extractor (highest precision), is also the most cautious verifier. This suggests a consistent ``personality'' characterized by epistemic caution.
...
Claims containing specific dates, quantities, or proper nouns proved difficult when web search results did not contain exact matches. The claim ``The Mayans invented the concept independently circa 4 A.D.'' (ground truth: \textit{Refuted}) produced three different verdicts: GPT-4 returned \textit{Insufficient Information}, Gemini correctly identified it as \textit{Refuted}, and DeepSeek incorrectly classified it as \textit{Supported}.
...
This thesis presented a comparative analysis of three Large Language Models---GPT-4, Gemini, and DeepSeek---within the TruthLens framework for automated fact-checking. Through rigorous experimentation across 1,950 API calls, we evaluated model performance on both claim extraction and verification tasks. This chapter summarizes the key findings, articulates the contributions of this work, offers practical guidance for deployment, and outlines directions for future research.
...
\textbf{Answer: GPT-4} emerges as the most effective model for claim extraction, achieving an F1-Score of \textbf{80.4\%} compared to 55.4\% (Gemini) and 65.3\% (DeepSeek).

The critical distinction lies in extraction strategy. GPT-4 adopts a \textit{balanced approach}, achieving 83.1\% precision and 77.8\% recall---correctly identifying approximately 8 out of 10 factual claims while maintaining high precision. In contrast, Gemini and DeepSeek employ a \textit{conservative strategy}, achieving exceptionally high precision (89.5\% and 95.8\% respectively) but at severe cost to recall (40.2\% and 49.6\%). These models fail to identify approximately half of all factual claims present in the input text.

For fact-checking pipelines where \textit{comprehensive coverage} is the priority, GPT-4 is the clear choice. The conservative models may be appropriate only in specialized contexts where false positives carry higher costs than missed claims.
...
\begin{itemize}
    \item \textbf{Gemini} achieves the highest accuracy (\textbf{82.0\%}), making it the optimal choice when overall correctness on representative datasets is the primary concern.
    \item \textbf{DeepSeek} achieves the highest Macro-F1 (\textbf{55.9\%}), indicating superior balanced performance across all verdict categories, including minority classes.
    \item \textbf{GPT-4} demonstrates the highest determinism (0.0\% standard deviation), producing identical results across independent runs.
\end{itemize}
...
\textbf{1. Over-Filtering in Extraction:} Gemini and DeepSeek frequently rejected context-dependent claims containing demonstrative pronouns (``This process...'', ``These results...''), despite containing verifiable information. This conservative behavior contributed to their low recall scores.
...
\textbf{1. Framework Adaptation and Extension:} We adapted the open-source ClaimeAI repository into the TruthLens framework, extending it to support multi-model comparative analysis. The engineering contributions include a unified LLM abstraction layer implementing the Strategy Pattern, custom middleware for DeepSeek's structured output requirements, and integration of the cost-effective Brave Search API for large-scale benchmarking.

\textbf{2. Empirical Comparative Analysis:} We conducted the first systematic comparison of GPT-4, Gemini, and DeepSeek within a decoupled two-phase fact-checking architecture. The experimental design---with $k=3$ runs for extraction and $k=2$ runs for verification across 1,950 total API calls---provides statistically reliable performance estimates with quantified variability. The results establish clear performance hierarchies: GPT-4 for extraction (balanced precision-recall), Gemini for verification accuracy, and DeepSeek for balanced multi-class verification.

\textbf{3. Methodological Contribution:} We demonstrated the value of decoupled evaluation in diagnosing LLM capabilities. By separating extraction from verification, we identified that the same model can exhibit different behavioral patterns across tasks (e.g., DeepSeek's conservative extraction correlates with cautious verification). This granular analysis would be impossible with monolithic end-to-end evaluation approaches.
...
\begin{itemize}
    \item \textbf{High-Coverage Fact-Checking:} Use GPT-4 for extraction to maximize claim identification (77.8\% recall), accepting moderate false positives.
    \item \textbf{High-Precision Applications:} Use Gemini or DeepSeek for extraction when false positives are costly, understanding that approximately half of factual claims will be missed.
    \item \textbf{Verification Tasks:} Use Gemini for highest overall accuracy (82.0\%) on representative datasets, or DeepSeek when balanced performance across verdict categories is required.
\end{itemize}

\textbf{Stability Considerations:} GPT-4 offers the most deterministic outputs ($\pm$0.0\% accuracy SD), making it suitable for applications requiring reproducible results. DeepSeek's higher variability ($\pm$4.6\% extraction F1 SD) suggests it may require additional engineering effort (e.g., ensemble approaches or multiple inference passes) to achieve production-level consistency.

\textbf{Cost-Accuracy Trade-offs:} Open-weights models like DeepSeek offer competitive performance at reduced API costs compared to proprietary alternatives. For budget-constrained deployments, DeepSeek provides a viable option, particularly for verification tasks where its Macro-F1 performance exceeds the other models.
...
This thesis demonstrates that current LLMs, when deployed within a structured agentic framework, can achieve human-comparable performance on straightforward factual claims. GPT-4's 80.4\% extraction F1 and Gemini's 82.0\% verification accuracy represent meaningful progress toward reliable automated fact-checking.
