...
\label{ch:background}
    ...
    \begin{enumerate}
        ...
        \textit{Adaptation:} We extended the search module to support the \textbf{Brave Search API} as a cost-effective, keyword-based retrieval backend, enabling experimental runs within the scope of this thesis. The implementation provides a unified interface that abstracts different search backends while maintaining consistent functionality.

        \item \textbf{Enhanced Verification Capabilities:}
        \textit{Original:} Verification was limited to Supported/Refuted verdicts.
        \textit{Adaptation:} We expanded the verification system to include \textit{Insufficient Information} verdicts, providing a more comprehensive fact-checking framework that better reflects real-world scenarios where evidence may be inconclusive.
...
\end{verbatim}
    ...
    The original repository included a search interface supporting expensive neural search APIs like Exa (formerly Metaphor) and Tavily. To enable large-scale benchmarking within the budget constraints of this study, this module was extended to support the \textbf{Brave Search API} \cite{BraveSearchAPI}.
