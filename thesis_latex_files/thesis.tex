...
\begin{titlepage}
    ...
    \begin{minipage}[t][0.9\textheight]{0.8\textwidth}
        \begin{flushleft}
            ...
            to Department VI - Computer Science and Media\\
...
\begin{abstract}
Large Language Models (LLMs) have transformed natural language processing, yet their tendency to generate factually incorrect information, known as hallucination, poses significant risks for applications requiring factual accuracy. This thesis presents a comparative analysis of three modern LLMs (gpt-4o-mini, gemini-2.5-flash, and deepseek-v3.2) within TruthLens, a custom implementation of a decoupled two-phase architecture adapted for multi-model benchmarking.

Evaluating performance on a benchmark derived from the BingCheck dataset, we identified distinct behavioral patterns across the models. For claim extraction, gpt-4o-mini achieves superior F1-Score (80.4\%) through a balanced precision-recall strategy, while gemini-2.5-flash and deepseek-v3.2 adopt conservative approaches with high precision but lower recall. For verification, models achieve 77--82\% accuracy on straightforward claims. While results suggest a performance drop on minority classes (Refuted and Insufficient Information), these findings should be interpreted as indicative trends due to the class imbalance in the benchmark.

Qualitative analysis highlights three key failure modes: the tendency to reject context-dependent claims, the generation of ineffective search queries, and the models' struggles with minority classes in imbalanced datasets. Despite these challenges, models achieve 69\% unanimous correctness on verification tasks, suggesting that LLM-based fact-checking shows promise for straightforward factual claims. This work contributes a reusable multi-model framework, empirical performance benchmarks, and actionable guidance for real-world deployment.
\end{abstract}
...
\begin{otherlanguage}{ngerman} 
    \begin{abstract}
    Large Language Models (LLMs) haben die Verarbeitung natürlicher Sprache grundlegend verändert. Ihre Neigung zur Generierung faktisch falscher Informationen -- bekannt als \textit{Halluzination} -- stellt jedoch ein erhebliches Risiko für Anwendungen dar, die faktische Genauigkeit erfordern.

    Diese Arbeit präsentiert eine vergleichende Analyse dreier LLMs (\texttt{gpt-4o-mini}, \texttt{gemini-2.5-flash} und \texttt{deepseek-v3.2}) im Rahmen von TruthLens, einer eigens entwickelten Zwei-Phasen-Architektur für Multi-Modell-Benchmarking. Die Evaluierung erfolgte anhand eines aus dem BingCheck-Datensatz abgeleiteten Benchmarks.

    Bei der Behauptungsextraktion erzielt \texttt{gpt-4o-mini} den höchsten F1-Score (80,4\%) durch eine ausgewogene Precision-Recall-Strategie. \texttt{gemini-2.5-flash} und \texttt{deepseek\-v3.2} verfolgen dagegen konservative Ansätze mit hoher Precision bei geringerem Recall. Bei der Verifikation erreichen alle Modelle 77--82\% Genauigkeit für eindeutige Behauptungen. Die Ergebnisse weisen auf Leistungseinbußen bei Minderheitsklassen hin, sind jedoch aufgrund der Klassenungleichheit als vorläufige Tendenzen zu werten.

    Die qualitative Analyse identifiziert drei zentrale Fehlermodi: die Ablehnung kontextabhängiger Behauptungen, ineffektive Suchanfragen sowie Schwierigkeiten mit unterrepräsentierten Klassen. Trotz dieser Einschränkungen erreichen die Modelle bei 69\% der Verifikationsaufgaben einstimmige Korrektheit. Dies deutet darauf hin, dass LLM-basierte Faktenprüfung für eindeutige faktische Behauptungen vielversprechend ist.

    Die Arbeit liefert ein wiederverwendbares Multi-Modell-Framework, empirische Leistungsbenchmarks sowie praxisorientierte Empfehlungen für den Einsatz solcher Systeme.
    \end{abstract}
\end{otherlanguage}
\clearpage
...
First and foremost, all praise and gratitude belong to Allah, the Most Gracious, the Most Merciful. Every success I have achieved is by His grace alone.
...
The advent of Large Language Models (LLMs) has changed the landscape of Natural Language Processing (NLP) and information retrieval. Models such as OpenAI's gpt-4o-mini \cite{GPT4omini}, Google's gemini-2.5-flash \cite{Gemini25}, and DeepSeek's deepseek-v3.2 \cite{DeepSeekV3.2} have demonstrated remarkable capabilities in generating human-like text, summarizing complex documents, and answering open-domain questions. However, the widespread adoption of these systems in critical domains—such as journalism, legal research, and education—is hindered by their tendency to generate plausible but factually incorrect information, a phenomenon widely known as ``hallucination'' \cite{Ji2023Survey}.

As LLMs are increasingly integrated into search engines, the ability to automatically verify the factual veracity of their outputs has become a paramount research challenge. This thesis addresses this challenge by developing TruthLens, a custom implementation of a decoupled two-phase architecture adapted from the open-source \textit{ClaimeAI} repository \cite{ClaimeAI_Repo}\footnote{Original repository available at: \url{https://github.com/BharathxD/ClaimeAI} (Accessed: 2025-12-22)}, for automated fact-checking and to benchmark the performance of three modern LLMs (gpt-4o-mini, gemini-2.5-flash, deepseek-v3.2) within this system.
...
\section{Motivation}
    ...
    The risk is not merely academic; in high-stakes applications, hallucinations can lead to the propagation of misinformation, legal liabilities, and the erosion of user trust. For instance, an LLM might generate a summary of a news article that accurately captures the tone but invents specific dates or statistics. This phenomenon—where text reads fluently but is factually incorrect—poses verification challenges.

    \subsection{The Need for Automated Fact-Checking}
    Manual fact-checking is unscalable given the volume of content generated by LLMs \cite{Metropolitansky2025, SAFE2024}. Traditional fact-checking pipelines often rely on human annotators or static knowledge graphs, which cannot keep pace with the dynamic nature of information on the web \cite{Cheng2024, Guo2022Survey}.
...
\section{Objective and Research Questions}
    ...
    The primary objective of this thesis is to perform a comparative analysis of three modern Large Language Models—\textbf{gpt-4o-mini}, \textbf{gemini-2.5-flash}, and \textbf{deepseek-v3.2}—to determine their effectiveness in an automated fact-checking pipeline.
    ...
    \begin{table}[H]
        \centering
            ...
            \hline
            DeepSeek & deepseek-v3.2 & \texttt{deepseek-chat} \\ 
            \hline
        \end{tabularx}
    ...
    \end{enumerate}
    This thesis aims to establish which of these three models offers the best balance between precision (trustworthiness) and recall (coverage) for building reliable automated fact-checking agents.
...
\section{Structure of the Thesis}
    ...
    \textbf{Chapter \ref{ch:implementation}} describes the technical implementation of the evaluation pipeline, including the Python-based agent architecture, the integration of LLM APIs, and the benchmarking scripts.
...
\label{ch:background}
    ...
    This thesis focuses specifically on detecting and mitigating \textbf{Factuality Hallucinations}. Unlike faithfulness errors, which can be mitigated through prompt engineering, factuality errors require external verification pipelines—such as the TruthLens framework proposed in this study (see Section \ref{sec:truthlens_framework})—to audit the model's output against reliable ground truth.
    ...
    \end{enumerate}
    This methodology is implemented in the ClaimeAI framework (see Section \ref{sec:truthlens_framework}), which forms the foundation of the extraction phase in this study. The underlying assumption is that higher-quality extraction leads to more accurate verification downstream.
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{|X|X|X|X|X|}
            \hline
            \textbf{answer\_id} & \textbf{question} & \textbf{sentence\_id} & \textbf{sentence} & \textbf{contains_factual_claim} \\
            \hline
    ...
    \end{enumerate}
    The authors of the SAFE framework \cite{SAFE2024} demonstrated that LLM agents using this method can match or exceed crowdsourced annotators in rating long-form factuality, while offering improved cost-effectiveness at scale. The TruthLens framework incorporates the SAFE-based verification agent from the ClaimeAI repository.
    
    \section{The TruthLens Framework}
    \label{sec:truthlens_framework}

    To empirically evaluate the performance of gpt-4o-mini, gemini-2.5-flash, and deepseek-v3.2, this thesis implements \textbf{TruthLens}. TruthLens is a specialized adaptation and extension of the open-source \textit{ClaimeAI} framework, with extensions to support systematic comparative multi-model benchmarking.

    \subsection{Foundation: The ClaimeAI Architecture}
    The architectural foundation of this study is \textbf{ClaimeAI} \cite{ClaimeAI_Repo}, an open-source fact-checking tool designed by BharathxD. ClaimeAI utilizes \textbf{LangGraph} \cite{LangGraph}, a library specifically designed for building stateful, multi-actor applications with LLMs. LangGraph enables the creation of cyclical graphs, allowing agents to loop, persist state, and make iterative decisions—a critical capability for the complex reasoning required in fact-checking tasks. Within this framework, the process is split into two autonomous agents:
    \begin{enumerate}
        \item \textbf{Claim Extractor:} Implements the Claimify methodology through a multi-node pipeline, with core transformations including Selection, Disambiguation, and Decomposition to isolate atomic claims (see Section \ref{ch:implementation} for the complete workflow).
    ...
    However, the original ClaimeAI repository was configured to use a single LLM provider (OpenAI's gpt-4o-mini) with the model identifier hardcoded in the configuration. While the underlying LangChain framework (via \texttt{init\_chat\_model}) theoretically supports multiple providers, the original implementation was not designed for controlled comparative benchmarking across multiple LLM providers with different API capabilities.

    \subsection{Scope of Adaptation}
    The original ClaimeAI repository was configured for single-provider execution with OpenAI's gpt-4o-mini hardcoded in the configuration. To conduct the multi-model comparative analysis required by this thesis, systematic extensions were necessary. These extensions address four key requirements: (1) ensuring experimental consistency across providers with different API capabilities, (2) enabling batch processing for statistical validation, (3) providing cost-effective search provider support, and (4) restoring the three-label verification schema. Chapter \ref{ch:methodology} details the methodological rationale for these requirements, and Chapter \ref{ch:implementation} describes their technical implementation.
...
\label{ch:methodology}
    ...
    While the original repository was configured for single-model execution (OpenAI's gpt-4o-mini), this study required implementation of controlled comparative benchmarking. Four methodological requirements drove the design of TruthLens:
    ...
    \begin{enumerate}
        ...
        \item \textbf{Batch Processing for Statistical Validation:}
        The original ClaimeAI was designed for interactive, one-claim-at-a-time processing. However, rigorous benchmarking requires processing $N=150$ sentences across 3 models with $k=3$ runs each—a total of 1,350 extraction API calls, plus 600 verification calls. This scale necessitates batch processing code with rate limiting (to respect API quotas), failure recovery (to handle transient network errors), and result serialization (to preserve data for analysis). Additionally, checkpointing mechanisms prevent data loss during long-running experiments.

        \item \textbf{Cost-Effective Search Provider Support:}
    ...
    \begin{itemize}
        \item \textbf{Phase 1 (Extraction):} The ground truth for extraction was derived from the expert annotations published by the authors of the \textit{Claimify} framework \cite{Metropolitansky2025}. Each of the 150 sampled sentences has a binary label (\textit{True/False}) indicating whether it contains a factual claim.
        
        \item \textbf{Phase 2 (Verification):} For the verification phase, a standardized benchmark of 100 atomic claims was required. The experimental procedure was as follows: (1) Phase 1 extraction was executed with all three models, (2) gpt-4o-mini was identified as the best-performing extractor (F1-Score = 80.4\%; see Table \ref{tab:extraction_results} in Chapter \ref{ch:results}), and (3) the first 100 claims from gpt-4o-mini's extraction output were selected as the Phase 2 benchmark dataset.

        This methodological choice ensures that all verifiers are evaluated on identical, well-formed atomic claims, prioritizing benchmark quality and reproducibility. While this introduces a potential limitation regarding claim characteristic generalization (discussed in Section \ref{sec:threats_validity}), it reflects the practical scenario where production systems would deploy the optimal extractor.

        The ground truth was then established via manual human annotation by the author using a \textbf{three-label schema}:
        \begin{itemize}
        ...
        The resulting class distribution (85 Supported, 3 Refuted, 12 Insufficient Information) became known only after completing manual annotation, as the benchmark claims had no pre-existing veracity labels. This constraint is discussed in Section \ref{sec:limitations}.
    \end{itemize}
...
This chapter details the technical realization of the TruthLens framework. It describes the system architecture, the provider management implementation for systematic multi-model benchmarking, and the development of the benchmarking pipeline used to execute the experiments.
    ...
    The initial prototype (based on the open-source \textit{ClaimeAI} repository) provided a baseline implementation configured for single-provider execution. To address the methodological requirements outlined in Section \ref{sec:experimental_framework}, the architecture was extended to support provider management, batch processing with statistical validation, and metric calculation.
    ...
    \begin{enumerate}
        ...
        \item \textbf{The Benchmarking Engine:} Pipeline for batch execution ($N=150$ samples, $k=3$ runs per phase, 3 models) with rate limiting, failure recovery, multi-run statistical analysis, and result serialization.
    \end{enumerate}
    ...
    The extraction pipeline is adopted directly from the ClaimeAI repository \cite{ClaimeAI_Repo}, which implements the Claimify methodology \cite{Metropolitansky2025}. \textbf{No modifications were made to the core extraction logic}; our adaptations (detailed in Section \ref{sec:experimental_framework}) occur at the provider management and batch processing layers, not within the pipeline itself.
    ...
    \section{Provider Management Implementation}
    ...
    The original repository included a search interface supporting expensive neural search APIs like Exa (formerly Metaphor) and Tavily. To enable the benchmarking experiments within the budget constraints of this study, this module was extended to support the \textbf{Brave Search API}.
    ...
    To conduct the comparative analysis, a benchmarking pipeline was developed to execute the experiments across the dataset.
    
    \subsection{Phase 1: Extraction Benchmarking}
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{|l|X|}
            ...
            \hline
        \end{tabularx}
        \caption[Extraction Dataset Schema]{Extraction Dataset Schema. The \texttt{[model]} prefix is replaced with provider names (e.g., \texttt{gpt4\_binary\_result}).}
        \label{tab:extraction_schema}
    \end{table}

    To ensure the statistical reliability defined in the methodology ($k=3$ runs), the benchmarking script includes a \texttt{generate\_unique\_filename} utility. This ensures that subsequent experimental runs do not overwrite previous data, versioning output files (e.g., \texttt{dataset\_run2.csv}, \texttt{dataset\_run3.csv}) to preserve a complete audit trail of all iterations.
    
    \subsection{Phase 2: Verification Benchmarking}
    ...
    \begin{table}[H]
        ...
        \begin{tabularx}{\textwidth}{|l|X|}
            ...
            \hline
        \end{tabularx}
        \caption[Verification Benchmark Schema]{Verification Benchmark Schema. The \texttt{[model]} prefix is replaced with provider names (e.g., \texttt{gpt4\_verdict}, \texttt{gemini\_reasoning}).}
...
This chapter presents the quantitative results of the comparative analysis. The experiments were conducted following the methodology defined in Chapter \ref{ch:methodology}, executing 1,950 pipeline invocations (1,350 for extraction across k=3 runs, and 600 for verification across k=2 runs), with each invocation involving multiple LLM API calls due to the multi-node agent architecture (consensus voting in extraction, iterative search in verification). All metrics are reported as mean values with standard deviation to demonstrate reliability across runs.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|X|X|}
        ...
        \hline
    \end{tabularx}
    \caption[Phase 1 Extraction Metrics (Positive Class)]{Phase 1 Extraction Metrics (Positive Class). Mean $\pm$ standard deviation across $k=3$ runs. $N=150$ sentences (78 positive, 72 negative).}
    \label{tab:extraction_results}
\end{table}

\textbf{Key Finding:} gpt-4o-mini achieves the highest F1-Score (80.4\%), demonstrating a balanced trade-off between precision and recall. In contrast, gemini-2.5-flash and deepseek-v3.2 exhibit a conservative extraction strategy, achieving high precision (89.5\% and 95.8\% respectively) at the cost of low recall (40.2\% and 49.6\%).
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|X|}
        ...
        \hline
    \end{tabularx}
    \caption[Phase 1 Extraction Metrics (Negative Class)]{Phase 1 Extraction Metrics (Negative Class). Specificity measures the ability to correctly identify non-factual content; NPV (Negative Predictive Value) indicates reliability when predicting ``no claim.''}
...
\begin{figure}[H]
    ...
    \caption[Precision-Recall trade-off for claim extraction]{Precision-Recall trade-off for claim extraction. gpt-4o-mini occupies the balanced region near F1=0.8, while gemini-2.5-flash and deepseek-v3.2 cluster in the high-precision, low-recall quadrant.}
...
\begin{figure}[H]
    ...
    \caption[Confusion matrices for claim extraction (averaged across $k=3$ runs)]{Confusion matrices for claim extraction (averaged across $k=3$ runs). gpt-4o-mini shows balanced errors, while gemini-2.5-flash and deepseek-v3.2 exhibit high false negative counts (47--39 missed claims).}
...
\begin{itemize}
    ...
    \item \textbf{gemini-2.5-flash:} Shows conservatism with only 4 false positives but 47 false negatives, missing over half of actual claims.
...
\begin{figure}[H]
    ...
    \caption[Run-to-run variability for extraction phase ($k=3$ runs)]{Run-to-run variability for extraction phase ($k=3$ runs). gpt-4o-mini demonstrates the most consistent performance with minimal variance.}
...
The verification phase evaluated each model's ability to assess the veracity of factual claims using search-augmented reasoning. A standardized benchmark of $N=100$ atomic claims was used, with ground truth labels distributed as: 85 \textit{Supported}, 3 \textit{Refuted}, and 12 \textit{Insufficient Information}. The severe underrepresentation of the Refuted class arose only after completing the manual annotation and is discussed in Section \ref{sec:limitations}.
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|X|X|X|X|X|X|}
        \hline
        \textbf{Model} & \textbf{Accuracy} & \textbf{Macro-F1} & \textbf{Supported F1} & \textbf{Refuted F1\textsuperscript{†}} & \textbf{Insufficient F1} \\
        \hline
        gpt-4o-mini & 79.0\% $\pm$ 0.0\% & 51.2\% $\pm$ 3.9\% & 89.2\% & 36.7\%\textsuperscript{†} & 27.8\% \\
        \hline
        gemini-2.5-flash & 82.0\% $\pm$ 1.4\% & 52.7\% $\pm$ 2.0\% & 90.9\% & 34.8\%\textsuperscript{†} & 32.5\% \\
        \hline
        deepseek-v3.2 & 77.5\% $\pm$ 2.1\% & 55.9\% $\pm$ 0.4\% & 87.3\% & 40.0\%\textsuperscript{†} & 40.5\% \\
        \hline
    \end{tabularx}
    \caption[Phase 2 Verification Metrics]{Phase 2 Verification Metrics. Mean $\pm$ standard deviation across $k=2$ runs. $N=100$ claims (85 Supported, 3 Refuted, 12 Insufficient).\\ \textsuperscript{†}Refuted F1 scores based on $n=3$ samples; highly sensitive to individual errors. Interpret with caution.}
...
The substantial gap between accuracy and Macro-F1 scores (approximately 25--30 percentage points) reveals a critical limitation: all models perform well on the majority class (\textit{Supported}, F1 $\approx$ 87--91\%) but struggle with minority classes (\textit{Refuted}, F1 $\approx$ 35--40\%; \textit{Insufficient}, F1 $\approx$ 28--41\%).
...
\begin{figure}[H]
    ...
    \caption[Per-class F1-scores for verification]{Per-class F1-scores for verification. The heatmap reveals consistently poor performance on minority classes (Refuted: $n=3$, Insufficient: $n=12$) compared to the majority class (Supported: $n=85$).}
    \label{fig:per_class_heatmap}
\end{figure}

\textbf{Important Caveat:} Given the extreme class imbalance (n=3 for Refuted), the per-class F1-scores for minority classes should be interpreted as indicative patterns rather than statistically reliable measurements. A single classification error in the Refuted class changes its F1-score by approximately 33\%. This limitation is acknowledged as a threat to validity (see Section \ref{sec:limitations}).
...
\begin{figure}[H]
    ...
    \caption[Verification performance comparison]{Verification performance comparison. The gap between Accuracy and Macro-F1 indicates that high accuracy is driven primarily by majority class performance.}
...
\begin{figure}[H]
    ...
    \caption[Inter-model agreement rates for verification verdicts]{Inter-model agreement rates for verification verdicts. Fleiss' $\kappa = 0.41$ indicates moderate agreement across all three models.}
...
\begin{figure}[H]
    ...
    \caption[Run-to-run variability for verification phase ($k=2$ runs)]{Run-to-run variability for verification phase ($k=2$ runs). gpt-4o-mini produces identical results across runs, while deepseek-v3.2 shows the highest variability.}
...
\begin{figure}[H]
    ...
    \caption[Radar chart comparing model performance across all key metrics]{Radar chart comparing model performance across all key metrics. gpt-4o-mini demonstrates the most balanced profile, while gemini-2.5-flash and deepseek-v3.2 show specialized strengths.}
    \label{fig:radar}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|l|X|X|X|}
        ...
        \hline
    \end{tabularx}
    \caption[Summary of model performance across both experimental phases]{Summary of model performance across both experimental phases. Bold indicates best performance for each criterion.}
    \label{tab:summary}
\end{table}
...
This chapter interprets the empirical findings presented in Chapter \ref{ch:results}, contextualizes them within the broader literature, and provides a qualitative analysis of model failure modes. To illustrate the behavioral differences: if these LLMs were members of a research team, \textbf{gpt-4o-mini} would be the balanced researcher who finds most evidence, while \textbf{gemini-2.5-flash} and \textbf{deepseek-v3.2} are the overly cautious analysts who only cite a fact if they are highly confident---even if it means ignoring half of the relevant material.

\section{Comparative Analysis of LLMs}

\subsection{Best Performing LLM for Extraction}

The extraction phase results identify \textbf{gpt-4o-mini} as the most effective model among those evaluated for claim extraction, achieving an F1-Score of 80.4\% compared to 55.4\% (gemini-2.5-flash) and 65.3\% (deepseek-v3.2). This superiority stems from gpt-4o-mini's balanced approach to the precision-recall trade-off.
...
\begin{itemize}
    ...
    \item \textbf{gemini-2.5-flash \& deepseek-v3.2 (Conservative):} Achieve high precision (89.5\% and 95.8\% respectively) but at severe cost to recall (40.2\% and 49.6\%). These models fail to identify approximately half of all factual claims present in the text.
\end{itemize}
...
The verification phase presents a more nuanced picture, with different models excelling on different metrics:\footnote{Conclusions about minority class performance (Refuted, Insufficient Information) are tentative due to small sample sizes (n=3 for Refuted, n=12 for Insufficient Information).}
\begin{itemize}
...
\textbf{Comparison with SAFE:} Wei et al. \cite{SAFE2024} reported that their SAFE method achieves 72\% agreement with crowdsourced human annotators, and in disagreement cases, SAFE outperformed crowdsourced human annotators 76\% of the time. Our results show that all three models achieve 77--82\% accuracy on claims where human annotators established ground truth, suggesting that LLM-based verification shows promise for practical deployment on majority-class claims, though challenges remain for minority classes.
...
Nevertheless, the 69\% unanimous correctness rate is a positive finding. It suggests that for straightforward factual claims with clear web evidence, current LLMs have achieved functional verification capability. The remaining 31\% of claims represent genuinely difficult cases---either due to ambiguous evidence, context-dependent claims, or limitations in web search coverage.
...
\textbf{1. Class Imbalance:} The verification benchmark contains 85 \textit{Supported}, 12 \textit{Insufficient Information}, and only 3 \textit{Refuted} claims. This severe imbalance occurred because the benchmark claims were not pre-labeled with veracity verdicts; the class distribution became known only after completing manual annotation. This constraint means that:
\begin{itemize}
...
\end{itemize}
Future benchmark construction should prioritize balanced sampling across verdict categories, either through stratified selection from pre-labeled datasets or iterative annotation until desired class proportions are achieved.
...
These architectural constraints directly inform our ethical approach: since the system cannot reliably handle minority classes or specialized domains without human oversight, deployment must prioritize transparency about these limitations to prevent over-reliance on automated verdicts.

\section{Ethical Considerations}

The development of automated fact-checking systems raises important ethical considerations. While this research aims to combat misinformation, the detailed analysis of LLM failure modes could theoretically inform adversarial actors seeking to craft claims that evade detection. We note that the failure patterns identified (context-dependent claims, specific date/quantity verification) represent known challenges in the field rather than novel vulnerabilities.

Furthermore, automated fact-checking systems should complement, not replace, human judgment---particularly for nuanced claims involving interpretation or contested evidence. The 31\% of claims where models disagreed underscores that human oversight remains essential for high-stakes applications. We advocate for transparent deployment that clearly communicates system limitations to end users, preventing over-reliance on automated verdicts.

\chapter{Conclusion and Future Work}
\label{ch:conclusion}

This thesis presented a comparative analysis of three Large Language Models---gpt-4o-mini, gemini-2.5-flash, and deepseek-v3.2---within the TruthLens framework for automated fact-checking. Through rigorous experimentation executing 1,950 pipeline invocations across both phases, we evaluated model performance on both claim extraction and verification tasks. This chapter summarizes the key findings, articulates the contributions of this work, offers practical guidance for deployment, and outlines directions for future research.
...
\begin{itemize}
    \item \textbf{gemini-2.5-flash} achieves the highest accuracy (\textbf{82.0\%}), making it the best choice among the evaluated models when overall correctness on representative datasets is the primary concern.
...
The substantial gap between accuracy (77--82\%) and Macro-F1 (50--56\%) across all models highlights the impact of dataset imbalance. While models perform well on the majority class (\textit{Supported}, F1 $\approx$ 87--91\%), the shortage of minority samples (n=3 for \textit{Refuted}) means that minority class findings should be considered preliminary observations requiring validation on larger, balanced datasets. This discrepancy suggests that while models show bias toward the majority class, their true capability in detecting misinformation requires further evaluation on more balanced datasets.
...
Despite these challenges, the models achieved 69\% unanimous correctness across all verification cases, demonstrating that for straightforward factual claims with clear web evidence, LLM-based verification can achieve reasonable reliability.

\section{Contributions}

This thesis makes three primary contributions to the field of automated fact-checking:

\textbf{1. Implementation for Systematic Multi-Model Benchmarking:} We extended the open-source ClaimeAI framework with code for controlled comparative analysis. The engineering contributions include:
\begin{itemize}
    ...
    \item Batch processing pipeline supporting systematic execution ($N=150$ samples, $k=3$ runs, 3 models) with rate limiting and failure recovery
    \item Integration of cost-effective Brave Search API for evidence retrieval
    \item Multi-run statistical validation framework for quantifying performance variability
\end{itemize}

\textbf{2. Empirical Comparative Analysis:} We conducted a systematic comparison of gpt-4o-mini, gemini-2.5-flash, and deepseek-v3.2 within a decoupled two-phase fact-checking architecture. The experimental design---with $k=3$ runs for extraction and $k=2$ runs for verification across 1,950 pipeline invocations---provides performance estimates with quantified variability. The results establish clear performance hierarchies: gpt-4o-mini for extraction (balanced precision-recall), gemini-2.5-flash for verification accuracy, and deepseek-v3.2 for balanced multi-class verification.
...
The rise of Large Language Models has amplified the challenge of misinformation while also providing new capabilities for automated fact-checking. These models generate fluent but sometimes factually incorrect content at scale, yet they also possess the reasoning capabilities to verify claims against external evidence.

This thesis shows that the evaluated LLMs, when deployed within a structured agentic framework, can achieve practical performance on straightforward factual claims. gpt-4o-mini's 80.4\% extraction F1 and gemini-2.5-flash's 82.0\% verification accuracy show that current LLMs can perform fact-checking tasks with reasonable accuracy, though with notable limitations on minority classes.

However, significant challenges remain. The struggle with minority classes, the sensitivity to search query quality, and the 31\% of claims where models disagree all indicate that LLM-based fact-checking is not yet ready to operate without human oversight. Crucially, while models achieved 69\% unanimous correctness, the remaining 31\% of ambiguous cases---where even these recent models produce conflicting verdicts---underscore that \textbf{human-in-the-loop validation remains essential} for high-stakes fact-checking applications. The path forward lies in hybrid systems that leverage LLM capabilities while acknowledging their limitations---using multi-agent architectures, diverse evidence sources, and human review for contested or high-consequence decisions.
...
\label{sec:appendix_availability}

The complete source code, experimental datasets, and reproduction scripts for this thesis are publicly available in a GitHub repository:

\begin{center}
\url{https://github.com/AasemElshahat/TruthLens}
\end{center}

A reproduction guide (\texttt{REPRODUCTION\_GUIDE.md}) is included in the repository, providing step-by-step instructions for:
\begin{itemize}
    \item Environment setup and dependency installation (Python 3.11+, Poetry)
    \item API key configuration for OpenAI, Google AI, DeepSeek, and Brave Search
    \item Dataset preparation and stratified sampling from BingCheck
    \item Execution of extraction and verification phases across all models
    \item Metrics calculation and statistical aggregation
    \item Figure generation for thesis visualizations
\end{itemize}

All experimental data files are preserved in the repository for independent verification:
\begin{itemize}
    \item \texttt{results/thesis\_datasets\_with\_LLM\_results/} --- Raw LLM outputs for all runs
    \item \texttt{results/extraction/} --- Per-run and aggregated extraction metrics
    \item \texttt{results/verification/} --- Per-run and aggregated verification metrics
    \item \texttt{thesis\_latex\_files/sources/ground\_truth\_data-BingCheck/} --- Source dataset
\end{itemize}

The repository is archived at the time of thesis submission to ensure long-term availability and version consistency for reproducibility purposes.
