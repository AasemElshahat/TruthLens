...
\label{ch:background}
    ...
    The architectural foundation of this study is \textbf{ClaimeAI} \cite{ClaimeAI_Repo}, an open-source fact-checking tool designed by BharathxD. ClaimeAI introduced the concept of a \textbf{decoupled pipeline} utilizing \textbf{LangGraph}, where the fact-checking process is split into two autonomous agents:
    \begin{enumerate}
    ...
    \subsection{From ClaimeAI to TruthLens: Key Adaptations}
    To answer the research questions of this thesis, significant architectural extensions were required to transform ClaimeAI from a single-provider configuration into a systematic multi-model benchmarking suite.

    The specific adaptations implemented in TruthLens include:

    \begin{enumerate}
        \item \textbf{Unified Provider Management Infrastructure:}
        \textit{Original:} ClaimeAI used LangChain's \texttt{init\_chat\_model} function for model initialization, with the model identifier hardcoded to OpenAI GPT-4 in the configuration file. While \texttt{init\_chat\_model} supports multiple providers through different model identifier strings, the original implementation lacked systematic provider management.

        \textit{Adaptation:} We developed a unified provider abstraction layer that standardizes LLM interactions across three different initialization approaches:
        \begin{itemize}
            \item \textbf{OpenAI:} Uses \texttt{init\_chat\_model} with standard configuration
            \item \textbf{Gemini:} Uses \texttt{ChatGoogleGenerativeAI} with provider-specific safety settings (HarmBlockThreshold.BLOCK\_NONE) to prevent false refusals on factual news content
            \item \textbf{DeepSeek:} Uses \texttt{ChatOpenAI} with custom endpoint (\url{base_url="https://api.deepseek.com"}) and JSON mode configuration
        \end{itemize}

        By implementing the Strategy Pattern \cite{Gamma1994}, we created a uniform interface that eliminates code-path asymmetry as a potential confounding variable in our comparative analysis. All three models pass through identical configuration logic, validation, and error handling, ensuring behavioral differences can be attributed to model capabilities rather than implementation artifacts.

        \item \textbf{Batch Benchmarking Engine:}
        \textit{Original:} Designed for interactive, one-claim-at-a-time processing without systematic evaluation infrastructure.
        \textit{Adaptation:} We developed a comprehensive batch-processing system capable of ingesting the BingCheck dataset ($N=150$), managing API rate limits, handling failures gracefully, and serializing results into structured CSV datasets for statistical analysis. This includes automated multi-run execution for statistical reliability and checkpointing mechanisms to prevent data loss during long-running experiments.

        \item \textbf{Expanded Search Provider Support:}
        \textit{Original:} Relies on expensive neural search providers (Tavily/Exa) without cost-effective alternatives.\\
        \textit{Adaptation:} We extended the search module to support the \textbf{Brave Search API} \cite{BraveSearchAPI} as a cost-effective, keyword-based retrieval backend, enabling experimental runs within the scope of this thesis. The implementation provides a unified interface that abstracts different search backends while maintaining consistent functionality.

        \item \textbf{Restored Verification Capabilities:}
        \textit{Original:} The ClaimeAI framework initially included support for \textit{Insufficient Information} verdicts (alongside \textit{Supported}, \textit{Refuted}, and \textit{Conflicting Evidence}), but this capability had been disabled (commented out) in the codebase prior to our fork. The verification workflow was effectively limited to binary classification.\\
        \textit{Adaptation:} We re-enabled the \textit{Insufficient Information} verdict category by restoring the previously commented-out enum value, providing a more comprehensive three-label fact-checking framework that better reflects real-world scenarios where evidence may be inconclusive. We removed support for the fourth category (\textit{Conflicting Evidence}) to align with standard fact-checking taxonomies and simplify the annotation process.

        \item \textbf{Multi-Run Statistical Analysis:}
        \textit{Original:} Single-run execution without statistical validation.
        \textit{Adaptation:} We implemented automated multi-run execution ($k=3$ for extraction, $k=2$ for verification) with statistical analysis capabilities to ensure result reliability and quantify model performance variability.
    \end{enumerate}


    \subsection{Implementation Approach}
    The TruthLens framework extends the original ClaimeAI codebase by building upon its LangGraph-based architecture while adding systematic provider management infrastructure. We developed a unified abstraction layer that standardizes interactions across providers with different API capabilities and configuration requirements. This design ensures experimental validity: all models are subject to identical initialization logic, error handling, and configuration management, eliminating implementation differences as potential confounding variables. The implementation preserves the core fact-checking methodologies established in the original framework while enabling rigorous comparative analysis.
...
\label{ch:methodology}
    ...
    While the original repository was configured for single-model execution (OpenAI GPT-4), this study required systematic infrastructure for controlled comparative benchmarking. The necessary extensions include:
    \begin{itemize}
        \item \textbf{Provider Management Infrastructure:} Developing a unified abstraction layer to standardize interactions across providers with different API capabilities (OpenAI's native structured output, Gemini's safety configuration requirements, and DeepSeek's custom endpoint and JSON mode limitations).
        \item \textbf{Batch Benchmarking Pipeline:} Implementing automated scripts for large-scale execution ($N=150$ samples, $k=3$ runs, 3 models) with rate limiting, failure recovery, and result serialization.
        \item \textbf{Statistical Validation Framework:} Developing multi-run execution with automated metric calculation to quantify performance variability.
        \item \textbf{Dataset Integration:} Adapting the BingCheck corpus for ground-truth validation of extraction and verification phases.
    \end{itemize}
...
This chapter details the technical realization of the TruthLens framework. It describes the system architecture, the engineering of the provider management infrastructure for systematic multi-model benchmarking, and the development of the automated benchmarking pipeline used to execute the experiments.
    ...
    While the initial prototype (based on the open-source \textit{ClaimeAI} repository) provided a baseline implementation configured for single-provider execution, this thesis required systematic infrastructure for controlled comparative benchmarking across multiple LLM providers. We extended the architecture to support provider management, batch processing with statistical validation, and automated metric calculation.
    ...
    To ensure experimental validity in our comparative analysis, we developed a unified provider management infrastructure that standardizes LLM interactions across all three models. While LangChain's \texttt{init\_chat\_model} function provides basic multi-provider initialization, our approach addresses two critical requirements for rigorous comparative benchmarking:

    \textbf{1. Experimental Consistency:} All models must pass through identical configuration, validation, and error-handling logic to eliminate implementation differences as confounding variables. Using different code paths for different providers (e.g., \texttt{init\_chat\_model} for OpenAI but direct provider classes for Gemini and DeepSeek) would introduce asymmetric treatment that could bias results.

    \textbf{2. Provider-Specific Adaptations:} Each provider has different API capabilities and requirements:
    \begin{itemize}
        \item \textbf{Gemini} requires safety setting overrides (\texttt{HarmBlockThreshold.BLOCK\_NONE}) to prevent false refusals on factual news claims—these settings aren't easily exposed through \texttt{init\_chat\_model}'s generic interface
        \item \textbf{DeepSeek} requires a custom API endpoint (\texttt{base\_url}) and lacks native support for LangChain's \texttt{with\_structured\_output()} method, necessitating custom middleware
    \end{itemize}

    By implementing the Strategy Pattern \cite{Gamma1994}, we created a design that balances experimental rigor (uniform treatment across providers) with practical requirements (provider-specific handling of API differences).
